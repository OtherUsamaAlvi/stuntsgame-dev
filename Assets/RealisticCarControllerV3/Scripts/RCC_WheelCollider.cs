...
public class RCC_WheelCollider : RCC_Core {
	...
	void FixedUpdate (){
        ...
        if (GetGroundMaterialIndex() == null)
        {
        }
        else
        {
	...
	private int GetGroundMaterialIndex(){
		...
		for (int i = 0; i < physicsFrictions.Length; i++) {
			if (wheelHit.collider != null)                                           // Added By Usama
			{
			...
			{
				Debug.Log("Exception!! RCC_WheelCollider Line 806 : Grownd Material Destroyed");                // Added By Usama
			}
		...
		if(!contacted){
			...
			for (int i = 0; i < RCCGroundMaterials.terrainFrictions.Length; i++) {
                ...
                {
					Debug.Log("Exception!! RCC_WheelCollider Line 829 : Grownd Material Destroyed");    // Added By Usama
				}
