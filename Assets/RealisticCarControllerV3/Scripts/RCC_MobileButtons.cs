...
public class RCC_MobileButtons : RCC_Core {
	...
	public RCC_UIController NOSButtonSteeringGyro;
	...
	void DisableButtons(){
		if (NOSButtonSteeringGyro)
			NOSButtonSteeringGyro.gameObject.SetActive(false);
		if (gasButton)
	...
	void EnableButtons(){
		...
		if (canUseNos) {
			if (NOSButtonSteeringGyro)
				NOSButtonSteeringGyro.gameObject.SetActive(true);
			if (NOSButton)
	...
	void Update(){
		...
		case RCC_Settings.MobileController.TouchScreen:
			...
			gyroInput = 0f;
				...
				if (NOSButtonSteeringGyro && NOSButtonSteeringGyro.gameObject.activeInHierarchy)
					NOSButtonSteeringGyro.gameObject.SetActive(false);
			if (steeringWheel && steeringWheel.gameObject.activeInHierarchy)
				steeringWheel.gameObject.SetActive(false);
			if (NOSButtonSteeringWheel && NOSButtonSteeringWheel.gameObject.activeInHierarchy)
					NOSButtonSteeringWheel.gameObject.SetActive(false);
		...
		boostInput = Mathf.Clamp((GetInput(NOSButtonSteeringGyro)+GetInput(NOSButton) + GetInput(NOSButtonSteeringWheel)), 0f, 1f);
