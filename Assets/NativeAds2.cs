...
public class NativeAds2 : MonoBehaviour
{
    [Header("Native Ads")]
    ...
    void Update()
    {
        ...
        if (AdsManager.Instance.sdkInitializes)
        {
            ...
            if (!string.IsNullOrEmpty(nativeAds1.adName) && !string.IsNullOrEmpty(adName))
            {                

                if (nativeAds1.adName == adName && !loadOnce)
                {                    
                    Debug.Log("native ads same" + " Native 1 Name:" + nativeAds1.adName + " Native 2 Name" + adName);
                    RequestNativeAd();
                    loadOnce = true;
                }                
            }       
